...
describe Command::Command, "upon instantiation" do
  it "should result non executed" do
...
describe Command::Command , "when unexecuting" do
  ...
  it "should not unexecute if not previously executed" do
...
describe Command::ProcCommand, "when unexecuting" do
  before do
    ...
    @non_unexecutable_command = Command::ProcCommand.new(lambda {true})
    
    @working_command.execute
    @failing_command.execute
    @non_unexecutable_command.execute
  end
  ...
  it "should not unexecute if the unexecute proc is not present" do
    @non_unexecutable_command.unexecute.should be_false
    @non_unexecutable_command.should be_executed
  end
end
